...
public class VideoCamera extends ActivityBase
	...
	private String mPreHighFrameRate = null;
	private String mPreVideoQuality = null;
    //
    ...
    private void initializeRecorder() {
        ...
        if (mVideoEncoder == MediaRecorder.VideoEncoder.H263 || mVideoEncoder == MediaRecorder.VideoEncoder.MPEG_4_SP) {
        ...
        if (mOrientation != OrientationEventListener.ORIENTATION_UNKNOWN) {
            if (info.facing == CameraInfo.CAMERA_FACING_FRONT) {
                rotation = (info.orientation - mOrientation + 180) % 360;
      ...
      // "adb shell setprop "debug.camera.yv12" true"
       ...
       if(yv12formatset.equals("true")) {
		...
		String VideoQuality = mPreferences.getString(
                CameraSettings.KEY_VIDEO_QUALITY,
                getString(R.string.pref_video_quality_default));
        if(!("off".equals(HighFrameRate))){
           ...
           if(mUnsupportedHFRVideoSize) {
			   ...
			   if ((mOnCreateDone == true) && (!mPreHighFrameRate.equals(HighFrameRate) || !mPreVideoQuality.equals(VideoQuality))) {
					String errorMsg = "HFR and VideoQuality are conflict! When HFR is on, only WQVGA and QVGA are supported!";
					Toast.makeText(VideoCamera.this, errorMsg, Toast.LENGTH_LONG).show();
				}
			}
	   ...
	   mPreHighFrameRate = HighFrameRate;
	   mPreVideoQuality = VideoQuality;
    ...
    private void showSharePopup() {
        ...
        if (!Util.isUriValid(uri, mContentResolver)) {
            Log.w(TAG, "showSharePopup(): Invalid URI");
            return;
        }
    ...
    private void storeImage(final byte[] data, Location loc) {
        ...
        if(-1 == orientation)
        {
            Log.i(TAG, "No oriention info in Exif, getting orientation from CameraInfo");
            orientation = 0;
        }
